...
//		bUseDataQ = TRUE;
	...
	do
	{
		...
		if (bUseDataQ)
		{
			...
			//DBGPRINT(RT_DEBUG_ERROR, ("MiniportMMRequest----------------tx ring FreeNum num:%ld---------------\n",FreeNum));
		}
		else
#endif /* RTMP_MAC_PCI */
		{
			FreeNum = GET_MGMTRING_FREENO(pAd);
			//DBGPRINT(RT_DEBUG_ERROR, ("MiniportMMRequest------------------mgmt FreeNum is %ld---------------\n",FreeNum));
		}
...
/*				Status = MlmeDataHardTransmit(pAd, QueIdx, pPacket); */
		...
		else
		{
			...
			{
				retryCnt--;
				DBGPRINT(RT_DEBUG_ERROR, ("retryCnt %d\n", retryCnt));
				if (retryCnt == 0)
				{
	...
	IN BOOLEAN CleanAll)
{
	...
	/*				RTMP_OS_Init_Timer(pAd, &pEntry->RetryTimer, GET_TIMER_FUNCTION(WPARetryExec), pAd);*/
			...
			/*******************************even add begin*************************************/
				extern UCHAR GLOBAL_AssocMac[6];
				extern UCHAR GLOBAL_AssocNum;
			
				COPY_MAC_ADDR(GLOBAL_AssocMac, pAddr);
				GLOBAL_AssocNum = 1;
