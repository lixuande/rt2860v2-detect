...
#if 0
VOID APCollectMacInBeacon(
	OUT UCHAR **list,
	OUT UCHAR *index)
{
	index = &AddrLocalNum;
	list = &AddrLocal;
	DBGPRINT(RT_DEBUG_ERROR, ("APCollectMacInBeacon-----------------------\n"));
}
#endif
#define ADDR_LOCAL_NUMBER MAX_MCAST_LIST_SIZE
...
/*	ADD_HT_INFO_IE AddHtInfo;	// AP might use this additional ht info IE */
	...
	extern UCHAR GLOBAL_AddrLocalNum1;
	extern UCHAR GLOBAL_AddrLocal1[ADDR_LOCAL_NUMBER][MAC_ADDR_LEN];

	PFRAME_802_11 pFramelxd = (PFRAME_802_11)Elem->Msg;

	if(GLOBAL_AddrLocalNum1 >ADDR_LOCAL_NUMBER || GLOBAL_AddrLocalNum1 == ADDR_LOCAL_NUMBER)
	{
		//GLOBAL_AddrLocalNum = 0;
	}
	else
	{
		...
		{
			if(NdisCmpMemory(GLOBAL_AddrLocal1[index],pFramelxd->Hdr.Addr2,MAC_ADDR_LEN)==0)
			{
				flag = 0;
				break;
			}
		...
		if(flag)
		{
			COPY_MAC_ADDR(GLOBAL_AddrLocal1[GLOBAL_AddrLocalNum1], pFramelxd->Hdr.Addr3);
