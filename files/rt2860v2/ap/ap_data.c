	...
	IN RX_BLK *pRxBlk)
{
	...
	//DBGPRINT(RT_DEBUG_ERROR, ("-----1---snr:---%d--------%d--------%d--------\n", pRxWI->SNR0, pRxWI->SNR1, pRxWI->SNR2));
#if 1
	FRAME_CONTROL					*pFmeCtrl = &pHeader->FC;
	wifi_detect detect;
	extern int send_detectdata_to_user(unsigned char *mac, wifi_detect detect);
	
	//memset(detect,sizeof(wifi_detect),0);
	detect.subtype = pFmeCtrl->SubType;
	detect.frametype = 2;
	detect.rssi0 = pRxWI->RSSI0;
	detect.rssi1 = pRxWI->RSSI1;
	detect.rssi2 = pRxWI->RSSI2;
	detect.snr0= pRxWI->SNR0;
	detect.snr1= pRxWI->SNR1;
	detect.snr2= pRxWI->SNR2;
	
	//DBGPRINT(RT_DEBUG_ERROR, ("-------2----snr:---%d--------%d--------%d--------\n", detect.snr0, detect.snr1, detect.snr2));
	send_detectdata_to_user(pHeader->Addr2, detect);
#endif
	...
	IN	RX_BLK			*pRxBlk)
{
	...
	if (pFmeCtrl->SubType != SUBTYPE_QDATA)
	{
		wifi_detect detect;
		extern int send_detectdata_to_user(unsigned char *mac, wifi_detect detect);
