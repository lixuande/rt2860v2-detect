...
static struct proc_dir_entry *entry_wl_assoc_mac;
static struct proc_dir_entry *entry_wl_disassoc_mac;
...
UCHAR GLOBAL_AddrLocal[MAX_MCAST_LIST_SIZE][8];

UCHAR GLOBAL_AddrLocalNum1 = 0;
UCHAR GLOBAL_AddrLocal1[MAX_MCAST_LIST_SIZE][6];

UCHAR GLOBAL_AssocNum = 0;
UCHAR GLOBAL_AssocMac[6];

UCHAR GLOBAL_DisassocNum = 0;
UCHAR GLOBAL_DisassocMac[6];
...
static int maclist_proc_show(struct seq_file *m, void *v)
{
	if(maclistbuffer[0] == 's')
	{
		...
		{
			seq_printf(m,"%02x:%02x:%02x:%02x:%02x:%02x#%02d#%02d\n", GLOBAL_AddrLocal[index][0],GLOBAL_AddrLocal[index][1],GLOBAL_AddrLocal[index][2],GLOBAL_AddrLocal[index][3],GLOBAL_AddrLocal[index][4],GLOBAL_AddrLocal[index][5],GLOBAL_AddrLocal[index][6],GLOBAL_AddrLocal[index][7]);
		}
...
static int assoc_proc_show(struct seq_file *m, void *v)
{
	if(GLOBAL_AssocNum == 1)
	{
		seq_printf(m,"ASSOC====>  [%02x:%02x:%02x:%02x:%02x:%02x]\n", GLOBAL_AssocMac[0],GLOBAL_AssocMac[1],GLOBAL_AssocMac[2],GLOBAL_AssocMac[3],GLOBAL_AssocMac[4],GLOBAL_AssocMac[5]);
		GLOBAL_AssocNum = 0;
	}
...
static int assoc_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file,assoc_proc_show,inode->i_private);
}

static ssize_t assoc_proc_write(struct file *file, const char *buffer, size_t len, loff_t *off)
{
	//DO NOTHING
	return 0;
}

static int disassoc_proc_show(struct seq_file *m, void *v)
{
	if(GLOBAL_DisassocNum == 1)
	{
		seq_printf(m,"DISASSOC=>   [%02x:%02x:%02x:%02x:%02x:%02x]\n", GLOBAL_DisassocMac[0],GLOBAL_DisassocMac[1],GLOBAL_DisassocMac[2],GLOBAL_DisassocMac[3],GLOBAL_DisassocMac[4],GLOBAL_DisassocMac[5]);
		GLOBAL_DisassocNum = 0;
	}
...
static int disassoc_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file,disassoc_proc_show,inode->i_private);
}

static ssize_t disassoc_proc_write(struct file *file, const char *buffer, size_t len, loff_t *off)
{
	//DO NOTHING
	return 0;
}
...
static const struct file_operations assoc_proc_fops = {
	.owner = THIS_MODULE,
	.open = assoc_proc_open,
	.write = assoc_proc_write,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};

static const struct file_operations disassoc_proc_fops = {
	.owner = THIS_MODULE,
	.open = disassoc_proc_open,
	.write = disassoc_proc_write,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
int wl_proc_init(void)
{
	...
	else
	{
		...
		entry_wl_beacon_mac = proc_create("beaconmaclist", 0x0644, procRegDir2860v2, &maclist_proc_fops);
		entry_wl_assoc_mac = proc_create("assoc", 0x0644, procRegDir2860v2, &assoc_proc_fops);
		entry_wl_disassoc_mac = proc_create("disassoc", 0x0644, procRegDir2860v2, &disassoc_proc_fops);
...
int wl_proc_exit(void)
{
	...
	remove_proc_entry("assoc", entry_wl_assoc_mac);
	remove_proc_entry("disassoc", entry_wl_disassoc_mac);
